...
**语言适配器（Language Adapter）**。核心包内置 Python、JavaScript/TypeScript、Java、Kotlin、Scala、Go、Rust、Ruby、
...
`build.sbt` -> `scala`，`requirements.txt` / `setup.py` / `pyproject.toml` -> `python`；未识别时默认 Python。
