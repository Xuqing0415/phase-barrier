...
- **v0.49.0 已完成**：语义级校验（结构校验之上的可选语义增强）——新增
  `anti_shortcut/semantic.py` 与第 5 类插件入口点组 `phase_barrier.semantic_validators`：
  需求追踪校验器（spec `REQ-001` 声明 -> 测试 `# REQ-001` 注释关联，阶段 2 推进时
  校验覆盖率，防 spec 与测试脱节）、Python AST 变异测试校验器（运算符 / 比较 /
  布尔确定性变异 + 工作区测试逐个运行变异体，阶段 4 推进时要求突变分数 >= 阈值，
  防空测试 / 假断言）、语义校验器插件契约与进程内注册 /
  `run_semantic_checks` 汇总（任一失败即阻止推进，插件异常按失败留痕不拖垮门禁）；
  官方 LLM 语义审查参考插件（`examples/semantic_llm_check/`，仅标准库、
  离线降级跳过、`network_required` 才阻断）；默认全部关闭不改变既有门禁行为；
  新增 `docs/semantic-validation.md` 与配置示例；tests/test_semantic.py 56 个用例。

- **v0.50.0 已完成**：深度补全第一层——spec 具体性与测试断言质量内置校验器。
  `spec_specificity`（阶段 1）五维分析（具体实体 / 接口签名 / 技术决策 /
  需求锚点命中 / 套话句式上限），直接拦截“307 字套话 spec”（套话样本实体 0 /
  签名 0 / 决策 0 / 套话 6 / 锚点 0，五个维度全灭并逐维度给出可操作消息）；
  需求锚点从用户原始需求提取 latin 标识符与中文双字领域词，过滤“实现 / 函数”等
  泛指词；用户需求缺失时锚点子检查自动关闭。`test_assertion_quality`（阶段 2，
  仅 Python）AST 识别纯常数断言（`assert True` / `assert 1 == 1`），strict 模式
  拒绝弱测试函数（消息含 `文件:函数` 与行号）；默认全部关闭不改变既有门禁；
  tests/test_semantic_depth.py 28 个用例，semantic 模块覆盖率 95%。

- **v0.51.0 已完成**：深度补全第二层——实现-文档双向追踪 + 断言目标数严格档。
  `implementation_traceability`（阶段 3，仅 Python）：spec 承诺的函数 / 类 / 字段 /
  REST 端点以整词 / 路径子串核对实现源码，缺失超过 `max_missing`（默认 0）即拒绝；
  实现里 spec 未承诺的公共符号默认 evidence 提示（`report_undeclared` 可关，不拦截）；
  单字符形参与 REQ 编号不追踪，避免误报。`test_assertion_quality.min_assert_targets`
  （默认 0 关闭）：每测试须覆盖至少 N 个不同断言目标（根标识符 / 被调函数），防
  “反复断言同一目标”的低价值测试。需求 -> spec（锚点）-> 测试（REQ / 断言质量）->
  实现（变异 / 追踪）的可验证关联链至此闭合；默认全部关闭不改变既有门禁；
  tests/test_semantic_trace.py 24 个用例，semantic 模块覆盖率 95%。
...
- 内部可执行项已清零：除既有批次（CI 平台矩阵真实工具链全量激活、插件自动发现 /
  ...
  具体性五维拦截套话、AST 拒绝纯常数断言），278 字量级套话 spec 与 `assert True`
  空壳测试不再能仅靠结构合规通过；v0.51.0 补上“深度补全第二层”（spec 承诺实体
  须在实现中落地，防答非所问；断言目标数严格档），需求 / spec / 测试 / 实现之间的
  可验证关联链闭合。
- 仍属外部 / 资源依赖、无法在本仓库单方面清零：
  - 自定义域名 `docs.phase-barrier.dev` 需用户先注册 / 解析域名再配置 DNS
    （截至 v0.48.3 公网 NXDOMAIN，未启用，可选）；
