...
累积了 v0.52.0–v0.62.0 的全部改动（各版本明细见下方条目，此处只做汇总与承诺）。

- **五道防线（v0.52.0–v0.56.0）**：需求模板 / 双模型交叉复核 / 约束 DSL + 形式化校验 /
  运行时行为审计 / 风险评分 + 概率人工复核；v0.60.0 补上**需求族风险累积**，堵住
  「把高风险任务拆成多个低风险子任务逐个过门禁」的 `risk_camouflage` 绕过。
- **红队 Agent（v0.60.0）**：18 类逃逸技术 + 沙箱回归，发现并修复真实逃逸
  （`truncate -s 0` 这类等价操作）。
- **形式化不变量（v0.58.0）**：TLA+ 模型检查 + Python 实现一致性测试，纳入 CI。
- **学习型防线（v0.59.0–v0.61.0）**：拦截案例库 → 模式提取 → 规则 / 提示词更新建议，
  以 PR + 人工审核方式合并。
- **SWE-bench 真实评测工具链（v0.61.0–v0.62.0）**：官方评测容器内跑 Agent、宿主侧官方
  harness 评分；Scale-20 实测 baseline 18/20 对 gated 20/20（78 次拦截、0 空补丁、
...
`PhaseBarrier` / `GateClient` / `GateDenied`）、配置 schema 的既有字段语义、
CLI 子命令与退出码、`.agent_gate/` 状态与证据文件格式。破坏性变更只会在 2.0 引入，
并提前一个 minor 版本给出弃用告警。详见 [docs/release.md](docs/release.md)。
...
- **feat: 深度补全第一层（v0.50.0）**：把“语义防穿”落到内置校验器，直接拦截
  “307 字套话 spec”与“assert True / assert 1 == 1 空壳测试”两类最典型的形式绕过。
- **spec 具体性五维校验**（`spec_specificity`，阶段 1）：对 spec 做五维分析——
  具体实体（`fib` / `/api/login` / `token=` / 反引号代码，默认 >= 5）、接口签名
  （def 行 / 函数·输入·输出·参数·返回·异常 / API 端点，默认 >= 2）、明确技术决策
  （“采用 X 避免 Y”，默认 >= 1）、需求锚点命中（从用户原始需求提取 latin 标识符与
  中文双字领域词并过滤“实现 / 函数”等泛指词，再核对 spec 是否逐条回应，默认 >= 2）、
  套话句式命中上限（默认 8 条空话正则，上限 1）。任一维度不达标即阻止阶段 1 -> 2；
  套话样本 spec 五个维度全灭（实体 0 / 签名 0 / 决策 0 / 套话 6 / 锚点 0），失败消息
  按维度给出当前值与下限及可操作建议。
- **测试断言质量校验**（`test_assertion_quality`，阶段 2，仅 Python）：AST 扫描
  test 函数，若函数含断言但全部为“纯常数断言”（不引用任何名称 / 属性 / 调用 /
  下标，如 `assert True` / `assert 1 == 1`）即标记为弱函数；`strict`（默认）下任一
  弱函数即拒绝，消息列出 `文件:函数` 与行号；`strict: false` 降级为仅警告。
- **配置与默认行为**：`GateConfig.semantic` 新增 `spec_specificity` /
  `test_assertion_quality`（默认全部关闭，不影响既有门禁）；`anti_shortcut init`
  注释模板同步；字段校验齐全（阈值非负 / 正则非空 / stages 0-6）；非法正则容错不崩溃。
- **测试**：新增 `tests/test_semantic_depth.py` 28 个用例（套话回归与富 spec 对照、
  锚点 / 实体 / 签名 / 决策 / 套话计数、非法正则容错、两校验器层与 Skill 端到端
  拦截 / 放行 / 默认关闭），本地全量相关测试通过；semantic 模块覆盖率 95%。
