...
def render_config(
    *,
    ...
    lines += [
        ...
        "#     max_missing: 0",
        "",
        "# ---- 五道防线（可选，v0.52.0，默认全部关闭）----",
        "# 在语义校验之上叠加多层防线，提升“隐性约束篡改”的逃逸成本（见 docs/five-lines.md）",
        "# defense:",
        "#   requirement_template:      # 防线1：需求模板（严格模式拒绝自由文本需求）",
        "#     enabled: true",
        "#     strict: true             # true 时缺模板即拒绝首个推进；false 时仅提示",
        "#   dual_review:               # 防线2：双模型交叉复核（需配置两个独立模型服务）",
        "#     enabled: true",
        "#     reviewer_a:",
        ...
        "#   formal_check:               # 防线3：形式化校验（constraints.yaml DSL + 可选 TLC）",
        "#     enabled: true",
        "#     require_tlc: false        # true 时缺少 TLC 将拒绝放行",
        "#   behavior_audit:             # 防线4：交付前审计 trace 是否违反 spec 禁止操作",
        "#     enabled: true",
        "#     deny_missing_test_command: false",
        "#   human_review:               # 防线5：交付前按风险分数概率抽样人工复核",
        "#     enabled: true",
        "#     sample_rate: 0.3",
